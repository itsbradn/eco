import { Interaction, InteractionTypes, Member, Message } from 'discordeno';
...
export interface InteractionCollector extends CollectInteractionOptions {
...
export interface CollectInteractionOptions extends BaseCollectorCreateOptions {
	/** Function that will filter messages to determine whether to collect this message */
	filter: (interaction: Interaction) => boolean;
}

export interface InteractionCollectorOptions extends BaseCollectorOptions {
	/** Function that will filter messages to determine whether to collect this message. Defaults to making sure the message is sent by the same member. */
	filter?: (interaction: Interaction) => boolean;
	/** The amount of messages to collect before resolving. Defaults to 1 */
	amount?: number;
...
export function collectInteractions(options: CollectInteractionOptions): Promise<Interaction[]> {
	return new Promise((resolve, reject) => {
		bot.collectors.interactions
			.get(options.key)
			?.reject('A new collector began before the user responded to the previous one.');

		bot.collectors.interactions.set(options.key, {
			...options,
			interactions: [],
			resolve,
			reject,
...
export async function needModalResponse(userId: bigint, options?: InteractionCollectorOptions) {
	const interactions = await collectInteractions({
		key: userId,
		createdAt: Date.now(),
		filter:
			options?.filter ||
			((interaction) => interaction.user.id === userId && interaction.type === InteractionTypes.ModalSubmit),
		amount: 1,
		duration: options?.duration || 1000 * 60 * 5,
	});
